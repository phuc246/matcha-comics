...
export const mockComics = Array.from({ length: 20 }, (_, i) => ({
  ...
  updatedAt: `${Math.floor(Math.random() * 24) + 1}h trước`,
  ...
  chapterCount: Math.floor(Math.random() * 400) + 10,
  isHot: i < 6,
  isNew: i >= 14,
...
export const mockNovels = Array.from({ length: 12 }, (_, i) => ({
  ...
  updatedAt: `${Math.floor(Math.random() * 12) + 1}h trước`,
  views: Math.floor(Math.random() * 10_000_000) + 100_000,
  rating: +(7.5 + Math.random() * 2.5).toFixed(1),
  status: i % 4 === 0 ? 'completed' as const : 'ongoing' as const,
  chapterCount: Math.floor(Math.random() * 2000) + 100,
  isHot: i < 4,
  isNew: i >= 9,
